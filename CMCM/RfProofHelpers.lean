...
-- Helper: a bottom event in b is in its own eventsAtEventEntry list.
private lemma bottom_event_mem_eventsAtEventEntry {b : Behaviour n} {e : Event n}
    (he_in_b : e ∈ b) (he_bottom : b.IsBottomEvent n e) : e ∈ b.eventsAtEventEntry n e := by
  simp only [Behaviour.eventsAtEventEntry]
  have hmem := b.bottom_e_in_b_impl_in_eventsAtEntryOfListBottomEvents n e.struct e.addr e
    he_in_b he_bottom rfl rfl
  rw [List.mem_map]
  exact ⟨⟨e, ⟨he_in_b, rfl, rfl⟩⟩, hmem, rfl⟩
...
private lemma dirEvent_down_true_ne_Vd_of_ne_Vd
    {de : DirectoryEvent n} {ds : DirectoryState n}
    (hd : de.down = true)
    (h_ne_Vd_dir : ∀ sv, ds ≠ DirectoryState.Vd sv)
    (hs_Vd : DirectoryState.toState n (de.SucceedingState n ds) = Vd) : False := by
  match hds : ds with
...
private lemma event_Vd_transition_implies_ncWrite_in_b
    ...
    (lin : ∀ e : Event n, CompoundProtocol.globalLinearizationEventOfRequest cmp b init e)
    -- Protocol property: each dir event de in b has requestDirectoryEvent (Axiom 3)
    -- linking de.eReq to de. Gives sameDown, dirReq, sameProtocol.
    (h_dir_req : ∀ de : DirectoryEvent n, Event.directoryEvent de ∈ b →
        b.requestDirectoryEvent n (init.stateAt n (Event.cacheEvent de.eReq)) true
          (Event.cacheEvent de.eReq) (Event.directoryEvent de))
    -- Each dir event's eReq is in b
    ...
    -- (5) If a dir event's req comes from reqToDirOfRequestEvent transforming a NC read on Vd
    ...
    | .inr ds =>
      ...
      have h_not_down : de_trans.down = false := by
        ...
        have hd : de_trans.down = true := by
          match de_trans.down, h with | true, _ => rfl | false, h => exact absurd rfl h
        exact dirEvent_down_true_ne_Vd_of_ne_Vd hd h_ne_Vd_dir hs_Vd
      ...
      -- Get requestDirectoryEvent from h_dir_req
      have hdir_req_de := h_dir_req de_trans hde_trans_in_b
      ...
      · -- de_trans.eReq IS a write: use it as witness
        refine ⟨Event.cacheEvent de_trans.eReq, h_eReq_in_b de_trans hde_trans_in_b, ?_, hce_not_down, ?_, ?_, ?_⟩
...
lemma stateAfter_Vd_implies_exists_ncWrite
  ...
  exact event_Vd_transition_implies_ncWrite_in_b he_in_list he_d_in_b he_d_isDir lin h_dir_req
    h_eReq_in_b h_cle_is_de h_s_dir h_not_global h_ncRead_prior_write hs_ne_Vd hs_Vd
