...
- **Dir state coherence from `dirAccessOfRequest`**: To prove dir state ≠ Vd (e.g., `onDirVd` vacuous), case-split on e_w's `dirAccessOfRequest`: `encapDir` → coherent request → dir at SW; `orderBeforeDir` → predecessor had perms, no downgrade between → dir stays SW; `orderAfterDir` → NC weak on Vd, no coherent perms to downgrade. The directory reflects the permission chain.
