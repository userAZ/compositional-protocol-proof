...
- **Don't add constraints that lose generality**: The original `isDirWrite` in `existsRClusterDirDown` was too restrictive — it only worked for scWriteDown but not scReadDown. The right constraint (`isDirRead`) works for scReadDown (the main RF case). When a definition has sorry's for certain cases, check if the definition's constraints match the PROTOCOL semantics rather than just one case.
