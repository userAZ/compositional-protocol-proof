...
4. **compose_three_compoundLin**: Uses 3-way invariant (`StepOrdering ∨ eq ∨ StepOrdering(reverse)`). The reverse case is NOT a band-aid — it's a necessary part of the inductive proof structure. The 2-way invariant (`StepOrdering ∨ eq`) would require proving acyclicity as a premise (circular). The reverse case from `dir_ordered` arises because local composition doesn't have the full cycle evidence to determine forward direction. It propagates until cycle closure resolves it. `cle_self_ordering_false` handles all three alternatives at closure in 2 lines.
