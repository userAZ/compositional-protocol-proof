...
- **Not all chains go through both CLEs.** For RF coherent writer: cmpLin_w = e_w (requestLin), chain goes e_w →(OB)→ e_r_down directly — CLE_w is NOT in the chain. For RF non-coherent: cmpLin_w is inside CLE_w (dirLin), chain goes cmpLin_w →(EncapBy)→ CLE_w →(OB)→ downgrade. EXAMINE EACH PROTOCOL SCENARIO to see which events are actually in the chain. Don't force a uniform cmpLin→CLE→middle→CLE→cmpLin template — the chain shape varies by scenario.
