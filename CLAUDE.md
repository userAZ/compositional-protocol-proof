...
- **Implement foundational fixes, not band-aids.** The reverse case in StepOrdering's 3-way invariant was a band-aid for `dir_ordered` giving the "wrong" direction. Instead of propagating the reverse case, the right fix was using StepOrdering for the reverse and delegating through the bridge — which eliminated 4 sorry's and simplified the proof. Band-aids create technical debt that compounds. A foundational fix (even if it requires more thought upfront) pays off immediately and prevents future blockers. ALWAYS ask: "Is this a band-aid or a real fix?" before implementing.
