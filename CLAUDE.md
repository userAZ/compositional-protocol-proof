...
- **CmpLinCleRel.cle_ob now carries h_not_dir.** `cle_ob` → cmpLin is NOT a directory event (from requestLin = cache event). This enables h_ne derivation for cle_ob+eq suffix case (cache ≠ dir). But cle_ob+cle_ob and inside+inside at same CLE still need event_oEnd_lt from edge context. Thread event_oEnd_lt through the construction chain or pass h_ne directly from the edge's event evidence.
