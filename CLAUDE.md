...
0d. **CRITICAL: DRAW THE PROTOCOL SCENARIO before proving.** Before filling any sorry or proving any lemma, DESCRIBE and DRAW the protocol scenario: what events are involved, what are their types (read/write/downgrade), what edges connect them, what temporal relationships exist. This catches bogus cases, reveals the real proof path, and prevents hours of mechanical grinding. Example: the CleLink.eq cycle closure was solved in minutes once I drew the junction diagram showing FR can't appear (read/write contradiction at junctions). **This is the single most useful proof strategy.**
