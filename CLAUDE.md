...
- **h_ne should come from OB/Encap/EncapBy, NOT event_oEnd_lt.** The proxy chain contains OB, Encap, EncapBy steps. Each gives strict inequality (different oStart). Use THESE for h_ne, not event_oEnd_lt. Prioritize OB > Encap > EncapBy (stronger first). `FinishesBefore` is weaker — only gives oEnd < oEnd, not oStart inequality. The clll/LinLink relations should use OB/Encap/EncapBy as the primary temporal evidence, with FinishesBefore as a fallback. This is more natural because OB/Encap/EncapBy directly name the temporal relationship between the proxy events.
