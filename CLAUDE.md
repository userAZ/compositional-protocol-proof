...
### Lessons learned (CmpLinForwardStep experimental approach, 2026-04-07)
- **CmpLinForwardStep (forward-or-eq + ProtoOBLevel) is STRICTLY STRONGER than the 3-way approach.** The main proof carries `forward ∨ eq ∨ reverse` and closes via ProtoOBLevel at cycle level. CmpLinForwardStep tries to eliminate the reverse at SINGLE-EDGE level, which requires "same CLE → same compoundLin" — true in the protocol but not provable from the axioms.
- **The gap: `compoundLinearizationEvent` is opaque.** `clusterDirectoryLinearizationEvent` depends ONLY on the CLE, so same CLE → same compoundLin. But `compoundLinearizationEvent` is an axiom field of CompoundProtocol that takes `e_creq` as parameter. The `choose` on its existential output varies with `e_creq`. Need either (a) a "same CLE → same compoundLin" axiom, or (b) accept the 3-way invariant.
- **The `inside × inside` case is the ONLY gap.** All other CmpLinCleRel pairs (eq/cle_ob/inside × eq/cle_ob/inside) are resolved: temporal contradictions for orderAfterDir (CLE.oEnd < e₂ < succ < CLE.oStart), finishesAfterProxy for cle_ob × cle_ob, isDirectoryEvent contradiction for inside + requestLin, direct TemporalRel for eq/cle_ob/inside × eq/cle_ob combos.
- **Helper lemmas for type conversion after `match` on Event.** `temporalRel_of_dirOB` lifts `DirectoryEvent.OrderedBefore` to `TemporalRel (.directoryEvent de₁) (.directoryEvent de₂)`. `lift_temporalRel_from_match` transports TemporalRel back through `hfc : cmpLin = .directoryEvent de` via subst. Avoids fighting Lean's match rewrites.
- **orderAfterDir temporal contradiction pattern.** For `cle_ob × (eq|inside)` with eventOB: CLE.oEnd < e₂.oStart (from cle_ob + OB), e₂.oEnd < succ.oStart (isSucc), succ.oStart < CLE.oStart (encapDir on successor). Chain gives CLE.oEnd < CLE.oStart → `Nat.lt_irrefl`. Works for both orderAfterDir branches (cle_ob × eq and cle_ob × inside).
- **Avoid `chain_of_sameCLE` when you can construct TemporalRel directly.** `chain_of_sameCLE` gives a 3-way (forward/eq/reverse). For cases like `cle_ob × cle_ob`, construct `finishesAfterProxy` directly: `BasicTemporalRel.finishesAfterProxy CLE h₂_ob (Nat.lt_trans h₁_ob (Event.oWellFormed n _))`. Avoids handling vacuous reverse branches.
...
- **After `match hfc : event, isDir with | .directoryEvent de, _ =>`: use helper lemmas.** `hfc` binds a match equation but `rw [hfc]` and `▸` often fail at term level because the match already substituted definitionally. Solution: write a small helper that takes the `DirectoryEvent` directly and converts types (e.g., `DirectoryEvent.OrderedBefore → Event.OrderedBefore`). Don't fight `▸`/`rw` through match bindings — extract the computation into a helper.
- **`CmpLinForwardStep` subtype**: `(TemporalRel cmpLin₁ cmpLin₂ ∨ eq) + ProtoOBLevel`. When `cmpLin₁ = cmpLin₂` (NC weak write/read writeback with same linearization point): compose via `heq ▸` substitution, DON'T try to construct `TemporalRel l l`. Composition is 4 lines. Irreflexivity is 1 line (proto_ob_level_irrefl).
- **`orderBeforeDir` contradiction with `CmpLinCleRel.eq`**: `eq` implies `dirLin → reqMissingPerms`. `orderBeforeDir` implies `reqHasPerms`. Two paths: (1) `dirLin`: `reqHasPerms_not_reqMissingPerms → False`. (2) `requestLin`: `compoundLin = e₂ (¬dir)` but `eq: compoundLin = CLE (dir)` → `e₂ = CLE → e₂ dir → ¬dir → False`.
- **Global SWMR vacuity**: `compound.globalSWMR : global.requests = {SCWrite, SCRead}`. NC requests ∉ SWMR. `orderAfterDir` at global level is vacuous. Use `global_protocol_of_gCacheOfCDir` for `protocol = .global`, then `eReqOfTheirProtocol` + `globalSWMR` to show req ∈ {SCWrite, SCRead} → coherent=true → ¬isNcWeak.
- **Use `TransGen CleLink` not `CleLink.trans`**: Adding `trans` to an inductive creates case-split obligations in every function that matches on it. Use `Relation.TransGen` for composition instead — it's free.
- **Cross-cluster TemporalRel via `finishesAfterProxy`**: For GLE₁ OB GLE₂: `CLE₁ →(finishesAfterProxy GLE₁, gleOB)→ GLE₂ →(finishesBefore)→ CLE₂`. Uses `gle_oEnd_lt_cle` (GLE.oEnd < CLE.oEnd). No CLE₁ OB CLE₂ needed.
