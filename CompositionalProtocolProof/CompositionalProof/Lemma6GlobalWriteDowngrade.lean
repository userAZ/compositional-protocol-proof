  ...
  | Sum.inr ds => match ds with
    ...
    -- Both end at Vc. Each branch does its own complete simp+rw chain.
      ...
      | ⟨some .wr, false⟩ =>
        ...
        -- Vd path: derive h_nc_write with full simp chain including cache state.
        have h_nc_write : (Event.req n e_dir_shim_acq).val.rw = .w ∧ (Event.req n e_dir_shim_acq).val.coherent = false := by
          ...
          simp [Behaviour.reqToDirOfRequestEvent, Event.reqToDirOfRequestEvent,
                hacq_req, hacq_state_before, Event.req, Event.down] at hacq_dir_req'
          -- hacq_dir_req' still has match on e_dir_shim_acq. Match to reduce.
          match e_dir_shim_acq, hfwd_mr_down_translation.acqDir.isDir with
          | .directoryEvent de, _ =>
            match e_shim_acq with
            | .cacheEvent ce_acq =>
              -- Fully concrete: ce_acq.req = acquire, cache state = Vd, ce_acq.down = false.
              have hacq_down := hfwd_mr_down_translation.acq.downgrade
              -- Convert hacq_req from Event to CacheEvent level
