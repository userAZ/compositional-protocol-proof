...
\begin{definition}[State (Def 5.1)]
  ...
  A \emph{state} consists of a permissions level $p$ (none, read, or write)
...
\begin{definition}[Request's Required State (Def 5.3)]
  ...
  is at least its Minimum Required State (MRS). For coherent requests this means
  the state is at least the MRS. For acquire/release/weak writes this additionally
  requires the state is coherent. For non-coherent weak reads this requires the
  state is not Vd.
\end{definition}
...
\begin{definition}[Global Linearization Point (Def 5.5)]
  ...
  A cluster request linearization event classifies where a cluster cache request
  linearizes:
  \begin{itemize}
    \item \textbf{At cache} (\texttt{clusterCacheLin}): the request has permissions and
      linearizes locally; the linearization event is the cache event itself.
    \item \textbf{At directory or beyond} (\texttt{clusterDirLin}): the request does not
