...
\begin{verbatim}
python3 model-check-axioms/scripts/run_axioms.py --clean --axioms-file axioms-to-run.txt
\end{verbatim}
...
The claims of our paper that correspond to this mechanization are summarized in the table below, which also explain how these can be checked/verified.
...
\begin{tabular}{ | c | c | }
  ...
  \hline \hline
    ...
    &
    \makecell{
      Read Section \ref{sec:readme-lean-mechianization} and Chapter \ref{ch:proof-overview}.
      \\
      Check in VSCode Theorem and Lemmas
      \\
      have blue checkmarks.
      \\
      Match paper definitions to Lean definitions.
      \\
      Comment: Those sections/chapter
      \\
      will help give a sense of how
      \\
      to navigate the Lean codebase.
      \\
      (i.e. where the definitions/theorem
      \\
      from the paper are located.)
      \\
  ...
  \hline
    ...
    &
    \makecell{
      Read Section \ref{sec:readme-murphi-case-studies}.
      \\
      Run Murphi Models.
      \\
      Comment: The mentioned Section provides
      \\
      instructions on how to use our
      \\
      scripts to run the axiom checks
      \\
      protocol with the Murphi Models.
    }
    \\
  \hline
\end{tabular}
...
\begin{tabular}{ | c | c | }
  ...
  \hline \hline
    ...
    &
    \makecell{
      The proof handles systems with both \\
      an SWMR and RCC (CPU and GPU) style protocols.\\
  ...
  \hline
    ...
    &
    \makecell{
      The proof (introduced in Chapter \ref{ch:proof-overview}) \\
      proves any requests (with preserved MCM ordering) \\
      from any cluster composed in our system \\
      satisfy their MCM. This allows any \\
      combination of cluster protocols.
    }
    \\  
  \hline
    \makecell{
    ...
    \makecell{
      The proof (introduced in Chapter \ref{ch:proof-overview}) \\
      states for any two requests (with \\
      with preserved MCM ordering) \\
      that are ordered, will \\
      linearize in order. \\
      This does not bar the core from \\
      issuing requests out-of-order or \\
      overlapping requests (for efficiency) \\
      if it doesn't need ordering \\
