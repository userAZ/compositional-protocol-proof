...
All 1 theorem(s) are sorry-free!
TODO: add murphi scripts here!
\end{verbatim}

You can also test this by adding something that depends on a sorried statement, e.g.
\begin{verbatim}
...
\textcolor{blue}{We've included a script to run the axiom checks on the
CXL and RCC-O murphi models for you. You can run them one by one, or 
try to run them in one go. If running them in one go fails, just run
the axiom model checking runs one by one.}

\textcolor{red}{TODO: Here we need to expalin what the script is and how to use it!}
...
URL (the ``http://0.0.0.0:8000/'' link after ``Serving'').
